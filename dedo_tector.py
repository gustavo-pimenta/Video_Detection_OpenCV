...
class video_input():
    ...
    def __init__(self):
        ...
        # variável booleana de controle de segurança
    ...
    def hand_detector(self):
        # importa as variáveis de cor de pele que foram criadas
        global lower_skin, upper_skin
 
        # Define o ROI (region of interest = região de interesse)
        # se trata da área onde a mão será detectada
        # é bom definir uma área para isso, pois processar a imagem inteira
        ...
        # Convertendo a imagem RGB para HSV para poder criar a máscara
        ...
        # Aplica Transformação Morfológica para filtrar o ruido do fundo da imagem
        ...
        # Aplicar método de Threshold para remover ruido
        ...
        try:
            ...
            # desenhar um retângulo azul em volta do contorno
            ...
            # Essa é a técnica responsável por descobrir a quantidade de dedos sendo mostrados
            ...
            for i in range(defects.shape[0]):
                ...
                # se o ângulo do defeito for maior que 90 graus, se trata do espaço entre dedos
                # serão desenhados círculos no inicio, fim e meio deste defeito
                if angle <= 90:
                    count_defects += 1
            ...
            # Salva o numero de dedos detectados em uma variável
            if count_defects == 0:
            ...
            # deixa a variável "No_hand" falsa pois a mão foi detectada
            self.NO_HAND = False
            ...
            # deixa a variável "No_hand" verdadeira pois a mão NÃO foi detectada
            self.NO_HAND = True
...
# função responsável por pegar os números extraídos e calcular a precisão deles
# mostrando para o usuário o quantidade de dedos que foi reconhecida e a precisão
# desse valor em porcentagem
def fingers_count():
 
    # importa a variável gatilho global
    global gat
    ...
    if gat >= 7: # ativa o loop apenas se o gatilho tiver o valor necessário
       
        # limpa o terminal para remover informação antigas
        # antes de inserir informações novas
        os.system('cls' if os.name == 'nt' else 'clear')
 
        # ativa o loop apenas se a mão for detectada
        if NO_HAND == False:
            try:
                # cria a variável soma
                sum = 0
                for i in fingers:
                    # soma as quantidade de dedos da lista
                    sum = sum + i
                # calcula a média da quantidade de dedos
                mean = int(sum / len(fingers))  
            except: pass
 
            # variavel para porcentagem
            perc = 0
            for i in fingers:
                # compara a media com a lista de dedos obtidos
                if mean == i:
                    perc += 1
            try:
                # divide o valor da comparação pelo numero de termos
                perc = perc / len(fingers)
                # multiplica por cem para obter porcentagem
                perc = perc*100
            except: pass
            # limpa a lista da quantidade de dedos
            video.fingers = []
            print('Lista de dados obtidos: ', fingers)
            print("Resultado:\nQUANTIDADE DE DEDOS EXIBIDOS: ", mean)
...
while True:
    ...
    # imterrompe o programa se pressionar ESC
    k = cv2.waitKey(30) & 0xff
    if k==27:
        break
   
# elimina o objeto de captura de video ao encerrar o programa
video.video.release()
# fecha todas as telas do OpenCV ao encerrar o programa
